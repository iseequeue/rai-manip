...
TimedPath PathFinder_RRT_Time::plan(const arr &q0, const double t0, const TimedGoalSampler gs, double tGoalLowerBound, double tGoalUpperBound){
  const bool fixedTime = true;//rai::getParameter<bool>("assembly/fixedTime", false); 
  const bool fixedTime = true;//rai::getParameter<bool>("assembly/fixedTime", false); 
  this->vmax = 0.25;
  ...
  while(true){
    ...
    if (this->check_planner_termination_condition()){
...
std::vector<std::pair<int, int>> PathFinder_SIRRT_Time::get_safe_intervals(const Eigen::VectorXd &qq) {
  ...
  for (int safe_int_id=0;safe_int_id<raw_safe_intervals.size();safe_int_id++){
     safeIntervals.emplace_back(std::ceil(raw_safe_intervals[safe_int_id].first),std::floor(raw_safe_intervals[safe_int_id].second));
  }
  ...
  // for(std::pair<double, double> safe_int:raw_safe_intervals){
  //   for (double t_frame=safe_int.first+1;t_frame<=safe_int.second;t_frame++){
  //     // double t_real = this->t_start + t_frame * dt;
  //     // double t_real = this->TP.min_time + t_frame ;
  //     if (!(TP.query(q, t_frame)->isFeasible)){
  //       // std::cout<<"q: "<<q<<std::endl;
  //       // // std::cout<<"t_real: "<<t_real<<std::endl;
  //       // std::cout<<"t_frame: "<<t_frame<<std::endl;
  //       std::cout<<"ACHTUNG!!! Collision in safe interval!!! t_frame= "<<t_frame<<" t_0, tmax = "<<this->t_start<< " "<<this->t_max <<std::endl;
  //       safeIntervals.clear();
...
bool PathFinder_SIRRT_Time::extend(Eigen::VectorXd &coords_of_new)
{
  ...
  coords_of_new = q_nearest->coords + delta_vector.normalized() * this->planner_range;
    ...
    arr start_q(q_nearest->coords.size());
    arr end_q(coords_of_new.size());
    for (int i=0; i<q_nearest->coords.size(); i++)
    {
      start_q(i) = q_nearest->coords(i);
      end_q(i) = coords_of_new(i);
    }
 
  arr free_coords = this->TP.steer_to_obstacle(start_q, end_q);
  if (free_coords == start_q){
    return false;
  }
  if (free_coords != end_q){
    for (int i=0; i<coords_of_new.size(); i++)
    {
      coords_of_new(i) = free_coords(i);
    }
  }
  return true;
}
...
std::vector<Vertex *> PathFinder_SIRRT_Time::set_parent(Eigen::VectorXd &coord_rand, std::vector<std::pair<int, int>> &safe_intervals_of_coord_rand)
{
    ...
    // std::cout<<"nearest nodes: "<<nearest_nodes.size()<<std::endl;
    // std::cout<<"safe ints "<<safe_intervals_of_coord_rand.size()<<std::endl;

    std::vector<Vertex *> added_vertices;
    int safe_interval_ind = 0;
    if (this->current_tree == this->start_tree)
    {
        // std::cout<<"ACHTUNG!!! start tree set parent!!!"<<std::endl;
        double time_to_goal = (coord_rand - this->goal_coords).norm() * (double)(1/this->dt) / this->vmax;
        ...
        for (std::pair<int, int> safe_int : safe_intervals_of_coord_rand) // For each interval
        {
            ...
            for (std::pair<Vertex *, int> candidate_node : nearest_nodes)
            {
                ...
                double time_to_node = (coord_rand - candidate_node.first->coords).norm() * (double)(1/this->dt) / (this->vmax);
            // std::cout<<"candidate_node: "<<candidate_node.first->safe_interval.first<<" "<<candidate_node.first->safe_interval.second<<std::endl;
                ...
                {
                    // std::cout<<"intervals don't overlap"<<std::endl;
                    continue;
                }
                ...
                if (this->is_collision_motion_with_static(start_coords,coord_rand)){
                  // std::cout<<"static collision - skip"<<std::endl;
                  continue;
                }
                // for each departure time
                for (double departure_time = std::max(candidate_node.first->arrival_time, (double)safe_int.first - time_to_node); departure_time <= std::min((double)candidate_node.first->safe_interval.second, (double)safe_int.second - time_to_node); departure_time += 25)
                {
                // std::cout<<"departure_time "<< std::max(candidate_node.first->arrival_time, (double)safe_int.first - time_to_node)<< " "<<departure_time  << " " <<std::min((double)candidate_node.first->safe_interval.second, (double)safe_int.second - time_to_node) <<std::endl;
                    ...
                    // std::cout<<"arrival_time: "<<arrival_time<<std::endl;
                    // std::cout<<"is_collision_motion: "<<is_collision_motion(start_coords, coord_rand, departure_time, arrival_time)<<std::endl;
    ...
    {
          // std::cout<<"goal_treee"<<std::endl;

        double time_to_start = (coord_rand - this->root_node->coords).norm() * (double)(1/this->dt) / (this->vmax);
        std::sort(nearest_nodes.begin(), nearest_nodes.end(), [](const std::pair<Vertex *, int> &a, std::pair<Vertex *, int> &b)
                  { return a.first->arrival_time > b.first->arrival_time; });
        // for (std::pair<Vertex *, int> candidate_node : nearest_nodes){
        //     std::cout<<candidate_node.first->arrival_time<<", ";
        ...
        for (std::pair<int, int> safe_int : safe_intervals_of_coord_rand) // For each interval
        {
            ...
            if ((safe_int.second - time_to_start) <= this->t_start )
            {
                  // std::cout<<"safe_int.second - time_to_start) <= this->t_start "<<std::endl;
            ...
            for (std::pair<Vertex *, int> candidate_node : nearest_nodes)
            {
                ...
                {
                  // std::cout<<"candidate_node.first->arrival_time - time_to_node < safe_int.first"<<std::endl;
                    break;
                }
                ...
                for (double departure_time = std::min(candidate_node.first->arrival_time, (double)safe_int.second + time_to_node); departure_time >= std::max((double)candidate_node.first->safe_interval.first, (double)safe_int.first + time_to_node); departure_time -= 25)
                {
                // std::cout<<"departure_time "<< std::min(candidate_node.first->arrival_time, (double)safe_int.second + time_to_node)<< " "<<departure_time  << " " <<std::max((double)candidate_node.first->safe_interval.first, (double)safe_int.first + time_to_node) <<std::endl;
...
bool PathFinder_SIRRT_Time::connect_trees(Eigen::VectorXd &coord_rand, std::vector<std::pair<int, int>> &safe_intervals_of_coord_rand, std::vector<Vertex *> another_tree_new_nodes)
{
    ...
    while (true)
    {
        ...
        if (!is_ok)
        {
          // std::cout<<"CONNECT EXTEND  is false"<<std::endl;
            break;
        }
          // std::cout<<"CONNECT EXTEND  is true"<<std::endl;
        ...
        if (safe_intervals_of_coord_rand.size() == 0)
        {
          // std::cout<<"CONNECT EMPTY SAFE INTERVALS"<<std::endl;
            break;
        }
        // std::cout<<"CONNECT SAFE INTERVALS NON ZERO"<<std::endl;
        this->swap_trees();
        std::vector<Vertex *> new_nodes = this->grow_tree(coord_rand, safe_intervals_of_coord_rand);
        this->swap_trees();
        if (new_nodes.size() == 0)
        {
          // std::cout<<"CONNECT NEW NODES EMPTY"<<std::endl;
            break;
        }
        // std::cout<<"CONNECT NEW NODES NON EMPTY"<<std::endl;
...
// Generates a random configuration for a robot with given joint limits
Eigen::VectorXd get_random_configuration(
    const std::vector<std::pair<double, double>>& robot_limits,
    int dof)
{
    ...
    for (int joint_ind = 0; joint_ind < dof; joint_ind++)
    {
        double random_value = dis(gen);
        result[joint_ind] = random_value * (robot_limits[joint_ind].second - robot_limits[joint_ind].first) + robot_limits[joint_ind].first;
    }
...
bool PathFinder_RRT_Time::check_planner_termination_condition() const
{
...
TimedPath PathFinder_SIRRT_Time::plan(const arr &q0, const double &t0, const arr &q_goal, const double &t_up){
  ...
  const auto rrt_duration = std::chrono::duration_cast<std::chrono::microseconds>(rrt_end_time - rrt_start_time).count();
  ...
  const bool fixedTime = rai::getParameter<bool>("assembly/fixedTime", false); 
    ...
    // std::cout << "start_safe_intervals: " << start_safe_intervals.size() << std::endl;
    if (start_safe_intervals.size() ==0){
      spdlog::error("safe_intervals is empty in the Initial point");
      TP.C.watch(true);
      return TimedPath({}, {});
    }
    this->start_tree->add_vertex(Vertex(q0_coords, start_safe_intervals[0]), nullptr, -1, t0);
    ...
    int max_goal_time = this->goal_safe_intervals.back().second;
    for (const std::pair<int,int> goal_safe_int:this->goal_safe_intervals){
    this->goal_tree->add_vertex(Vertex(q_goal_coords, goal_safe_int), nullptr, -1, goal_safe_int.second);
    ...
    std::vector<std::pair<double, double>> robot_limits;
      arr lims = this->TP.C.getLimits();
      if (lims.d0 ==0){
        std::cout<<"ERROR: NO LIMITS FOR JOINTS SPECIFIED"<<std::endl;
        assert(lims.d0>0);
      }
      for (uint i=0; i<lims.d0; ++i){
        // std::cout<<lims(i, 0)<< " " << lims(i, 1) << std::endl;
        robot_limits.push_back(std::pair<double, double>(lims(i, 0),lims(i, 1)));
      }
    ...
    {        
        ...
        for (int i = 0; i < qs.d0; i++)
        {
          coord_rand(i) = qs(i);
        }
        // coord_rand  = get_random_configuration(robot_limits,q0.d0);
        // std::cout << coord_rand << std::endl;
